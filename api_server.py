...
# Nihai model: Kavram Darbogazi (Concept Bottleneck, V2_CB1).
# Eski fuzyon modeli (B3) geriye donuk uyumluluk icin yedek olarak denenir.
DEFAULT_CHECKPOINT = Path("out/experiments_v2/V2_CB1/checkpoints/best.pt")
FALLBACK_CHECKPOINT = Path("out/experiments/B3/checkpoints/best.pt")
DEFAULT_CALIBRATION = Path("out/experiments/B3/calibration.json")
...
def _resolve_checkpoint(explicit: Optional[str | Path]) -> Optional[Path]:
    ...
    if env:
        ...
        if p.is_file():
            return p
    # Frozen/dev uyumlu: model/ bundle -> out/ dev yollari
    try:
        from src.app_paths import resolve_model_checkpoint
        resolved = resolve_model_checkpoint()
    ...
    return FALLBACK_CHECKPOINT if FALLBACK_CHECKPOINT.is_file() else None


def _is_concept_bottleneck(ckpt_path: Path) -> bool:
    """Checkpoint Kavram Darbogazi modeli mi (mode/num_concepts iceriyor mu)?"""
    try:
        import torch
        ck = torch.load(ckpt_path, map_location="cpu", weights_only=False)
        return ("mode" in ck) or ("num_concepts" in ck)
    except Exception:
        return False
...
def create_app(
    ...
    preload_model: bool = False,  # geriye donuk uyumluluk (desktop_app)
) -> FastAPI:
    ...
    async def lifespan(_app: FastAPI):
        if resolved_ckpt is None:
            ...
            try:
                if _is_concept_bottleneck(resolved_ckpt):
                    # Nihai model: Kavram Darbogazi (Concept Bottleneck)
                    from src.inference.pipeline_v2 import InferencePipelineV2
                    from src.app_paths import resolve_feature_stats
                    stats_path = resolve_feature_stats() or "out/real/feature_stats_v2.json"
                    state["pipeline"] = InferencePipelineV2(
                        resolved_ckpt, device=resolved_device, enable_llm=enable_llm,
                        feature_stats_path=stats_path,
                    )
                    state["model_type"] = "concept_bottleneck_v2"
                    logger.info("Kavram Darbogazi pipeline yuklendi: ckpt=%s device=%s", resolved_ckpt, resolved_device)
                else:
                    # Geriye donuk uyumluluk: eski fuzyon modeli (B3)
                    from src.inference.pipeline import InferencePipeline
                    cal_path = DEFAULT_CALIBRATION if DEFAULT_CALIBRATION.is_file() else None
                    state["pipeline"] = InferencePipeline(resolved_ckpt, device=resolved_device, enable_llm=enable_llm, calibration_path=cal_path)
                    state["model_type"] = "fusion_legacy"
                    logger.info("Eski fuzyon pipeline yuklendi: ckpt=%s device=%s", resolved_ckpt, resolved_device)
    ...
    async def health():
        return {
            ...
            "llm_available": bool(state["pipeline"] and state["pipeline"].llm and getattr(state["pipeline"].llm, "is_available", False)),

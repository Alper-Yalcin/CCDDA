"""
train_kido.py  —  Model A: KIDO + qwen_selected

manifest_kido.csv ile EfficientNet-B0 tabanli ClinicalFusionClassifier egitimi.
Klinik ozellik: sifir (image-only baseline, V1 ile karsilastirma icin).
...
# -- Config -------------------------------------------------------------------
TRAIN_MANIFEST = Path("out/manifest_kido.csv")
TEST_MANIFEST  = Path("out/manifest_qwen.csv")
OUT            = Path("out/kido_run")
...
def make_transforms():
    train_tf = transforms.Compose([
        ...
        transforms.ColorJitter(brightness=0.35, contrast=0.35, saturation=0.25, hue=0.07),
...
def main():
    ...
    print(f"[init] device={DEVICE}  Model A: KIDO+qwen")

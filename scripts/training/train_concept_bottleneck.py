"""
Concept Bottleneck egitimi — klinik bilgiyi yapisal yapan model.

kayip = CE(emotion, y) + concept_weight * MSE(tahmin_kavram, gercek_kavram)
  gercek kavram = standartlastirilmis klinik ozellikler (validity maskeli)

Kullanim:
  python -m scripts.training.train_concept_bottleneck --exp-id CB1 --mode bottleneck
  python -m scripts.training.train_concept_bottleneck --exp-id CB2 --mode hybrid
"""
...
def run_epoch(model, loader, ce, optimizer, train, concept_weight):
    model.train() if train else model.eval()
    preds, tgts = [], []
    tot_ce, tot_cl = 0.0, 0.0
    ctx = torch.enable_grad() if train else torch.no_grad()
    with ctx:
        for b in loader:
            img = b["image"].to(DEVICE, non_blocking=True)
            true_c = b["clinical_features"].to(DEVICE, non_blocking=True)  # standartlastirilmis
            val = b["clinical_validity"].to(DEVICE, non_blocking=True)
            y = b["label"].to(DEVICE, non_blocking=True)

            logits, pred_c = model(img, return_concepts=True)
            loss_ce = ce(logits, y)
            # Kavram kaybi: sadece gecerli kavramlar uzerinde MSE
            diff = (pred_c - true_c) ** 2 * val
            loss_concept = diff.sum() / val.sum().clamp(min=1.0)
            loss = loss_ce + concept_weight * loss_concept

            if train:
                optimizer.zero_grad(); loss.backward(); optimizer.step()
            tot_ce += float(loss_ce.item()) * y.size(0)
            tot_cl += float(loss_concept.item()) * y.size(0)
            preds.extend(logits.argmax(1).cpu().tolist()); tgts.extend(y.cpu().tolist())
    n = len(tgts) or 1
    return {"ce": tot_ce/n, "concept": tot_cl/n,
...
def main():
    ...
    print(f"=== {args.exp_id} === mode={args.mode} concept_w={args.concept_weight}", flush=True)
    ...
    model = ConceptBottleneckClassifier(backbone=args.backbone, mode=args.mode).to(DEVICE)
    ...
    for epoch in range(1, args.epochs + 1):
        tr = run_epoch(model, train_loader, ce, optimizer, True, args.concept_weight)
        va = run_epoch(model, val_loader, ce, optimizer, False, args.concept_weight)
        scheduler.step()
        print(f"  ep{epoch:02d} tr_f1={tr['macro_f1']:.4f} val_f1={va['macro_f1']:.4f} "
              f"concept_mse={va['concept']:.3f} ({time.time()-t0:.0f}s)", flush=True)
        ckpt = {"model_state": model.state_dict(), "epoch": epoch,
                "val_macro_f1": va["macro_f1"], "backbone": args.backbone, "mode": args.mode}
    ...
    res = {
        "exp_id": args.exp_id, "mode": args.mode, "backbone": args.backbone,
        "concept_weight": args.concept_weight,
        "best_val_f1": round(float(best_f1), 4), "train_time_s": round(time.time()-t0, 1),
    ...
    print(f"\n{args.exp_id} ({args.mode}): Test F1={res['test_macro_f1']:.4f} Acc={res['test_acc']:.4f}", flush=True)
    for c in CLASSES:

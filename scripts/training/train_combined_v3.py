"""
train_combined_v3.py — EfficientNet-B2 + Gercek etiket + Pseudo etiket birlesik egitim
...
V3 farki: EfficientNet-B2 (1408 cikis) + buyuk goruntu (260px) + daha dusuk LR

Cikti: out/combined_v3_run/
"""
...
def make_transforms():
    train_tf = transforms.Compose([
        transforms.Resize(288), transforms.RandomCrop(260),
    ...
    val_tf = transforms.Compose([
        transforms.Resize(288), transforms.CenterCrop(260),
        transforms.ToTensor(), transforms.Normalize(MEAN, STD),
    ])
...
def load_pseudo_records() -> list[dict]:
    if not PSEUDO_CSV.exists():
        print(f"[hata] {PSEUDO_CSV} bulunamadi")
        return []
...
def main():
    ...
    print(f"[init] device={DEVICE}  model=EfficientNet-B2")
    ...
    model     = EfficientNetB2Classifier(num_classes=NUM_CLASSES, pretrained=True).to(DEVICE)

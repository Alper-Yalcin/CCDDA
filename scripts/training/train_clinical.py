"""
train_clinical.py  —  Model B: Orijinal qwen_selected + OpenCV klinik ozellikler

ClinicalFusionClassifier'in klinik MLP kolu gercek OpenCV ozellikleriyle beslenir.
V1 ile temel fark: klinik vektor artik sifir degil.

Test seti: manifest_qwen.csv'deki 212 orijinal qwen test ornegi.
Ozellikler: out/features_qwen.csv (18 OpenCV klinik ozellik, validity %100)

Cikti: out/clinical_run/
"""
...
FEATURES_CSV = Path("out/features_qwen.csv")
OUT          = Path("out/clinical_run")
...
def class_weights_tensor(labels, num_classes):
...
def run_epoch(model, loader, criterion, optimizer, train: bool):
    ...
    return {"loss": total_loss/n, "acc": float(accuracy_score(tgts_all, preds_all)),
...
def main():
    ...
    print(f"[init] device={DEVICE}  Model B: qwen + klinik ozellikler")
    print(f"[features] {FEATURES_CSV}")

    train_tf, val_tf = make_transforms()

    # Dataset - klinik ozellikler ile
    train_ds = SigLIPDrawingDataset(MANIFEST, "train", train_tf, features_csv=FEATURES_CSV)
    val_ds   = SigLIPDrawingDataset(MANIFEST, "val",   val_tf,   features_csv=FEATURES_CSV)
    test_ds  = SigLIPDrawingDataset(MANIFEST, "test",  val_tf,   features_csv=FEATURES_CSV)
    print(f"[data] train={len(train_ds)}  val={len(val_ds)}  test={len(test_ds)}")

    # Klinik ozellik gecerlilik kontrolu
    sample_batch = train_ds[0]
    feat_valid_rate = sample_batch["clinical_validity"].mean().item()
    print(f"[features] ornek gecerlilik orani: {feat_valid_rate:.3f}")
    ...
    results = {"accuracy": float(acc), "precision": float(prec), "recall": float(rec),
               "macro_f1": float(f1),
